...
| **2. `sc_local_agent_runtime` + `sc_qwen_core`** (`sce.local-agent`) | ✅ `3a47eca`, signed (operating knowledge + observe tools; Win32 mesh tools deferred to Tier-H) | ✅ 2026-09-01 — local qwen (no CC) read 3 real SC files, wrote a grounded `observation.md`, **[completed]**, VERIFIED, 4.4 min, 0 denials. (1st run hit a Spark connection reset on the final turn → model_error → drove the `39f954c` bounded-retry fix; clean re-run.) | 🟡 **PARTIAL — verified by Ron 2026-09-01**: operating knowledge + observe tools ported; mesh actions (`spawn_peer`, `send_role_message`, `capture_role_window`) deferred to #6. Not a plain check until #6 lands. |
| **3. crypto/identity/governance kernel** (`sc_identity`/PCTC, `sc_firewall`, `sc_pq`, migration/assignment) | ⛔ **HALTED — blocking security finding (awaiting Ron's decision)**: the source `DelegationToken.verify()` does NOT authenticate caveats cross-agent (root Ed25519 sig covers root fields only; chain_mac unchecked) → caveat removal escalates scope, verify still passes. Reproduced live. Porting it would assert a false guarantee. AgentIdentity (Ed25519) + ProvenanceLedger (hash-chain) are sound; the token needs a fix decision. Detail + repro + fix options: `Owner's Inbox/2026-09-01_LAYER3_BLOCKING_security_finding_PCTC_delegation.md`. MELD gate honored (no format in repo/docs/commits). | | |
